...
import "../interfaces/IAntagiRouter.sol";
import "../interfaces/IAntagiBondCurve.sol";
import "../interfaces/IAntagiFactory.sol";
import "../libraries/Math.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "./AntagiERC20.sol";

/// @title The Antagi pair contract template
/// @author NatX
/// @notice This contract when deployed enables users to swap their NFTs for ETH and vice versa
/// @dev This contract should be deployed by the Antagi factory contract
contract AntagiPair is AntagiShares {
    ...
    IAntagiRouter public router; // the Router contract instance
    address public routerAddress; // the Router contract address

    IAntagiBondCurve public curve; // the bonding curve contract instance
    address public curveAddress; // the bonding curve contract address

    IAntagiFactory public factory; // the Factory contract instance
    ...
    function initialize(address _router, address _curve, address _tokenAddress, address _owner, uint256 _fee) external onlyFactory {
        require(_fee <= 50, "Antagi: Fee must be less thhan or equal to 5%");
        require(initialized == false, "Antagi: Can not initialize a contract more than once");
        factory = IAntagiFactory(factoryAddress);
        routerAddress = _router;
        router = IAntagiRouter(_router);
        curveAddress = _curve;
    ...
    function adjustFee(uint256 _fee) external {
        require(msg.sender == owner, "Antagi: Only owner can call this function");
        require(_fee <= 50, "Antagi: Fee must be less thhan or equal to 5%");
        feeMultiplier = _fee;
    }
    ...
    function addLiquidity(uint256[] calldata tokenIds, address _from) external payable onlyRouter {
        ...
        require(_from == owner, "Antagi: Access not granted");
        ...
        if (reserve0 == 0 || reserve1 == 0) {
            ...
            require(reserve0 * msg.value == reserve1 * tokenIds.length, "Antagi: Operational Error");
            batchTransferFrom(tokenIds, _from);
    ...
    function removeLiquidity(uint256[] memory tokenIds, address _to) external {
        ...
        require(shares <= balanceOf[_to], "Antagi: Amount exceeded");
    ...
    function swap(uint256[] memory tokenIds, address to) external payable returns(uint256) {
        if (msg.value == 0) { // calculate fee before hand, use another check
            ...
            require(msg.value >= (amountIn + fee + platformFee), "Antagi: Invalid Price");
    ...
    modifier onlyFactory {
        require(msg.sender == factoryAddress, "Antagi: Only Factory can call this function");
        _;
    }

    modifier onlyRouter {
        require(msg.sender == routerAddress, "Antagi: Only Factory can call this function");
        _; 
    }
}
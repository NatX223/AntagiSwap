...
async function main() {
  ...
  const factory = await hre.ethers.deployContract("AntagiFactory", [feeToSetter]);
  ...
  const router = await hre.ethers.deployContract("AntagiRouter", [factory.target, feeToSetter, bondingCurve.target]);
